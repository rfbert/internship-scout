 ...
 *
 * It lives here rather than in `prisma/seed.ts` because two callers need it:
 * the `prisma db seed` CLI, and the demo reset endpoint. A copy in each place
 * is how the two drift until "reset" stops restoring what the seed builds.
 *
 * Idempotent throughout — upserts and existence checks — so running it against
 * a database that already holds the dataset reuses it instead of stacking a
 * second copy.
 */
import type { PrismaClient } from "@prisma/client";
import { DEFAULT_WEIGHTS } from "@/lib/constants";
import { normalizeCompany } from "@/lib/normalize";
import { seedDemo } from "@/server/demo/dataset";

/* The seeded identity comes from the environment so no personal data lives in
   the repo. Set SEED_USER_EMAIL (and friends) to seed your own account; the
   defaults are neutral placeholders. CI passes unset repo variables through as
   EMPTY strings, so fall back on blank too — `??` alone would seed a blank
   identity and an Invalid Date. */
...
export async function seedAll(prisma: PrismaClient): Promise<void> {
  ...
  // ── Demo dataset ──────────────────────────────────────────────────────
  // Invented listings, scored by the real engines. See ./dataset.ts.
